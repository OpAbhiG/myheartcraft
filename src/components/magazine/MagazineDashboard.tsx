...
import { LayoutDashboard, BookOpen, Plus, Eye, Copy, Edit3, Trash2, Search, Share2, FolderOpen, Check } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background text-on-background flex flex-col lg:flex-row antialiased relative" id="magazine-dashboard">
      ...
      <nav className="hidden lg:flex flex-col h-screen p-8 bg-white/70 backdrop-blur-md w-64 fixed left-0 top-0 z-40 border-r border-primary/10">
        <div className="mb-12">
          ...
          <div className="font-label-caps text-[9px] text-on-surface-variant mt-1.5 uppercase tracking-[0.25em] opacity-75 font-bold">Magazine Studio</div>
        </div>
      ...
      <main className="flex-grow w-full lg:ml-64 p-6 md:p-12 pb-24 md:pb-12 min-h-screen relative z-10">
        ...
        <div className="mb-12 animate-fade-in flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <p className="text-[10px] uppercase tracking-[0.3em] font-bold text-secondary mb-3">AI editorial publishing</p>
        ...
        <div className="lg:hidden flex gap-3 mb-8 overflow-x-auto pb-2 border-b border-primary/10">
          <button onClick={onCreateNew} className="text-xs font-bold uppercase tracking-wider px-4 py-2 border border-primary bg-primary text-white rounded-xl">New Magazine</button>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20 border border-dashed border-primary/20 rounded-3xl bg-white/50">
            ...
            <h3 className="font-display text-xl text-primary font-bold mb-1">No magazines found</h3>
                  ...
                  <div className="h-44 bg-primary/5 border-b border-primary/10 relative overflow-hidden flex items-center justify-center rounded-t-3xl">
                    {coverPhoto ? (
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center p-4 text-center bg-gradient-to-br from-rose-50 to-amber-50">
                        <span className="font-display text-lg font-semibold italic text-primary/65">{p.basicInfo.title}</span>
                        <span className="text-[9px] uppercase tracking-wider text-primary/40 mt-2 font-mono">{getCategoryLabel(p.category)}</span>
                      </div>
                    )}
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <p className="text-xs text-on-surface-variant line-clamp-2">
                        Created for {p.recipientName || 'Someone Special'}
                      </p>
                    </div>
