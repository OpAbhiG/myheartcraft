...
import { LayoutDashboard, BookOpen, Plus, Eye, Share2, Copy, Trash2, Search, Check, FolderOpen, Edit3 } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background text-on-background flex flex-col lg:flex-row antialiased relative" id="open-when-dashboard">
      ...
      <nav className="hidden lg:flex flex-col h-screen p-8 bg-surface-container w-64 fixed left-0 top-0 z-40 border-r border-primary/25">
        <div className="mb-12">
          ...
          <div className="font-label-caps text-[9px] text-on-surface-variant mt-1.5 uppercase tracking-[0.25em] opacity-75">Open When Studio</div>
        </div>
        ...
        <ul className="flex-1 space-y-3">
          ...
          <li>
            <button onClick={onNavigateToMagazine} className="w-full flex items-center gap-3 px-4 py-3 text-on-surface-variant border border-transparent hover:border-primary/20 hover:bg-background rounded-none text-left transition-all">
              <BookOpen className="w-4 h-4 text-on-surface-variant/75" />
      ...
      <main className="flex-grow w-full lg:ml-64 p-6 md:p-12 pb-24 md:pb-12 min-h-screen">
        ...
        <div className="mb-12 animate-fade-in flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <p className="text-[10px] uppercase tracking-[0.3em] font-bold text-[#999] mb-3">Sealed Emotional Gifts</p>
        ...
        <div className="lg:hidden flex gap-3 mb-8 overflow-x-auto pb-2 border-b border-primary/10">
          <button onClick={onCreateNew} className="text-xs font-bold uppercase tracking-wider px-3 py-1.5 border border-primary bg-primary text-background">New Collection</button>
          ...
          <button onClick={onNavigateToMagazine} className="text-xs font-bold uppercase tracking-wider px-3 py-1.5 border border-primary/20 text-on-surface">Magazine Maker</button>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20 border border-dashed border-primary/20">
            ...
            <h3 className="font-display-lg text-xl text-primary font-normal mb-1">No collections found</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {filtered.map(p => (
              <div key={p.id} className="glass-card flex flex-col border border-primary h-[390px] overflow-hidden group">
                ...
                <div className="h-44 bg-surface-container-high border-b border-primary/20 relative flex items-center justify-center p-6 text-center bg-radial from-amber-50 to-orange-50">
                  <div className="space-y-1.5">
                    <span className="font-mono text-[9px] uppercase tracking-widest text-primary/60">OPEN WHEN...</span>
                    ...
                    <span className="text-[8px] uppercase tracking-widest text-[#999] block font-mono">Occasion: {OCCASION_LABELS[p.occasion] || p.occasion}</span>
                  </div>
                  ...
                  <span className="absolute bottom-3 left-4 font-mono text-[8.5px] uppercase tracking-wider bg-black/60 text-white px-2 py-0.5">
                ...
                <div className="p-5 flex-1 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-center text-[10px] text-on-surface-variant/75 font-mono mb-2">
                      ...
                      <span>{p.views || 0} recipient views</span>
                    </div>
                    <div className="text-xs text-on-surface-variant leading-relaxed">
                      <strong>Opened by recipient:</strong> {getOpenedCount(p)} of {p.messages.length} messages read.
                    </div>
                    {p.enableAnalytics && getOpenedCount(p) > 0 && (
                      <span className="text-[9.5px] text-[#666] font-mono block mt-1">Analytics tracking enabled</span>
                    )}
                  </div>

                  <div className="mt-4 pt-4 border-t border-primary/10">
                    <div className="text-[10px] text-on-surface-variant/75 font-mono mb-3 text-right">
                      Last edited: {new Date(p.updatedAt).toLocaleDateString()}
                    </div>
                    
                    <div className="grid grid-cols-5 border border-primary divide-x divide-primary bg-background">
                      <button
