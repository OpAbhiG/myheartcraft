...
import { LayoutDashboard, BookOpen, Plus, Eye, Link2, Edit3, Trash2, Calendar, User, Copy, Check, Search, Share2, FolderOpen, RefreshCw, FileText, ChevronRight } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background text-on-background flex flex-col lg:flex-row antialiased relative" id="scrapbook-dashboard">
      ...
      <nav className="hidden lg:flex flex-col h-screen p-8 bg-surface-container w-64 fixed left-0 top-0 z-40 border-r border-primary/25">
        <div className="mb-12">
          ...
          <div className="font-label-caps text-[9px] text-on-surface-variant mt-1.5 uppercase tracking-[0.25em] opacity-75">Scrapbook Studio</div>
        </div>
      ...
      <main className="flex-1 w-full lg:ml-64 p-6 md:p-12 pb-24 md:pb-12 min-h-screen">
        ...
        <div className="mb-12 animate-fade-in flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <p className="text-[10px] uppercase tracking-[0.3em] font-bold text-[#999] mb-3">Creative memory making</p>
        ...
        <div className="lg:hidden flex gap-3 mb-8 overflow-x-auto pb-2 border-b border-primary/10">
          <button onClick={onCreateNew} className="text-xs font-bold uppercase tracking-wider px-3 py-1.5 border border-primary bg-primary text-background">New Scrapbook</button>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20 border border-dashed border-primary/20">
            ...
            <h3 className="font-display-lg text-xl text-primary font-normal mb-1">No albums found</h3>
                ...
                <div key={p.id} className="glass-card flex flex-col border border-primary h-[390px] overflow-hidden group">
                  ...
                  <div className="h-44 bg-surface-container-high border-b border-primary/20 relative overflow-hidden flex items-center justify-center">
                    {firstPhoto ? (
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center p-4 text-center bg-radial from-amber-50 to-orange-100/50">
                        <span className="font-display text-lg font-semibold italic text-primary/45">{p.title}</span>
                        <span className="text-[9px] uppercase tracking-wider text-primary/30 mt-2 font-mono">{getTypeLabel(p.type)}</span>
                      </div>
                    )}
                    {/* Badge */}
                    ...
                    <span className="absolute bottom-3 left-4 font-mono text-[8px] uppercase tracking-wider bg-black/60 text-white px-2 py-0.5 rounded-none">
                      {p.size} Size
                    </span>
                  </div>
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span className="text-[9.5px] font-sans text-on-surface-variant flex items-center gap-1">
                          <Eye className="w-3.5 h-3.5" /> {p.views || 0} views
                        </span>
                      </div>
