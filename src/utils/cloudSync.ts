...
// --- Magazine Cloud APIs ---
export async function syncMagazineToCloud(project: MagazineProject): Promise<void> {
  if (!project || !project.id) return;
  try {
    const res = await fetchWithRetry(PRIMARY_CLOUD_URL, { cache: 'no-cache' }, 3, 500);
    if (res.ok) {
      const json = await res.json();
      const currentMags = Array.isArray(json.magazines) ? json.magazines : [];
      const idx = currentMags.findIndex((m: any) => m.id === project.id);
      if (idx >= 0) {
        currentMags[idx] = project;
      } else {
        currentMags.push(project);
      }
      await fetchWithRetry(PRIMARY_CLOUD_URL, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...json,
          magazines: currentMags
        })
      }, 3, 500);
    }
  } catch (e) {
    console.warn('Sync magazine to cloud warning:', e);
  }
}

export async function fetchMagazineFromCloud(id: string): Promise<MagazineProject | null> {
  try {
    const res = await fetchWithRetry(PRIMARY_CLOUD_URL, { cache: 'no-cache' }, 3, 500);
    if (res.ok) {
      const json = await res.json();
      if (json && Array.isArray(json.magazines)) {
        return json.magazines.find((m: any) => m.id === id) || null;
      }
    }
  } catch (e) {
    console.warn('Fetch magazine from cloud warning:', e);
  }
  return null;
}

// --- Scrapbook Cloud APIs ---
...
// --- Open When Cloud APIs ---
export async function syncOpenWhenToCloud(project: OpenWhenProject): Promise<void> {
  if (!project || !project.id) return;
  try {
    const res = await fetchWithRetry(PRIMARY_CLOUD_URL, { cache: 'no-cache' }, 3, 500);
    if (res.ok) {
      const json = await res.json();
      const currentOWs = Array.isArray(json.openwhens) ? json.openwhens : [];
      const idx = currentOWs.findIndex((o: any) => o.id === project.id);
      if (idx >= 0) {
        currentOWs[idx] = project;
      } else {
        currentOWs.push(project);
      }
      await fetchWithRetry(PRIMARY_CLOUD_URL, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...json,
          openwhens: currentOWs
        })
      }, 3, 500);
    }
  } catch (e) {
    console.warn('Sync openwhen to cloud warning:', e);
  }
}

export async function fetchOpenWhenFromCloud(id: string): Promise<OpenWhenProject | null> {
  try {
    const res = await fetchWithRetry(PRIMARY_CLOUD_URL, { cache: 'no-cache' }, 3, 500);
    if (res.ok) {
      const json = await res.json();
      if (json && Array.isArray(json.openwhens)) {
        return json.openwhens.find((o: any) => o.id === id) || null;
      }
    }
  } catch (e) {
    console.warn('Fetch openwhen from cloud warning:', e);
  }
  return null;
}
